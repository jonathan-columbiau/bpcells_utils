...
```{r}
FindMarkerGenes = function(ref_bpcells, ref_metadata, tree, n_genes = 5, metadata_cluster_column = "cluster_label", metadata_cell_label_column = "cell_label") {
  ...
  for (i in 1:length(internal_nodes)) { #iterate over each parent node
    ...
    for (j in 1:length(child_node_round_robin_matchups)) {
      ...
      cells_node1 <- ref_metadata[ref_metadata[,metadata_cluster_column] %in% c(node1_tip_nodes),metadata_cell_label_column]
      cells_node2 <- ref_metadata[ref_metadata[,metadata_cluster_column] %in% c(node2_tip_nodes),metadata_cell_label_column]
 ...
 #1) Normalize reference atlas.
  ...
  for (i in 1:length(marker_genes)) {  #Iterate through parent nodes
    for (j in 1:length(marker_genes[[i]])) { ##2) Iterate through matchups
      ...
      subset_dataset <- subset_dataset %>% write_matrix_dir(tempfile("classify_mat"))
      #perform pca
...
Classify <- function(bpcells_query, models, tree_struc) {
  ...
  # save to disk to make it quick
  query_cells <- bpcells_query %>% write_matrix_dir(tempfile("mat"))
...
ref_bpcells <- LoadBPCellsObj("../../")
pseq_sample <- LoadBPCellsFromTable("~/Desktop/Patch-Seq/Batch 1-2 results/mm39-5xFAD alignment/salmon.merged.gene_counts.tsv", dir_name = "~/pseq_sample")
