...
```{r}
FindMarkerGenes = function(ref_bpcells, ref_metadata, tree, n_genes = 5, metadata_cluster_column = "cluster_label") {
  ...
  ref_bpcells <- ref_bpcells %>% write_matrix_dir(tempfile("mat"), compress = F)
  ...
  for (i in 1:length(internal_nodes)) { #iterate over each parent node
    ...
    for (j in 1:length(child_node_round_robin_matchups)) {
      ...
      cells_node1 <- ref_metadata$cell_label[ref_metadata[,metadata_cluster_column] %in% c(node1_tip_nodes)]
      cells_node2 <- ref_metadata$cell_label[ref_metadata[,metadata_cluster_column] %in% c(node2_tip_nodes)]
      subset_atlas <-ref_bpcells[, c(cells_node1, cells_node2)] %>% transpose_storage_order()
      celltype_labels <- c(rep(node1, length(cells_node1)), rep(node2, length(cells_node2))) %>% as.factor()
      ...
      pairwise_markers %<>% filter(foreground_mean > 1 |background_mean > 1) %>% select(-background) %>% distinct(feature, .keep_all = TRUE) %>% mutate(log2_fc = log2(foreground_mean/background_mean))
...
```{r}
GetModels <- function(marker_genes, ref_bpcells, ref_metadata, tree, metadata_cluster_label = "cluster_label") {
 #1) Normalize reference atlas.
  # Normalize by reads-per-cell
  ref_bpcells <- multiply_cols(ref_bpcells, 1/Matrix::colSums(ref_bpcells))
  ...
  ref_bpcells %<>% t() %>%  write_matrix_dir(tempfile("mat"))
  for (i in 1:length(marker_genes)) {  #Iterate through parent nodes
    for (j in 1:length(marker_genes[[i]])) { ##2) Iterate through matchups
      ...
      #if on tip node
      cells_node1 <- ref_metadata$cell_label[ref_metadata[,metadata_cluster_label] %in% node1_tip_nodes]
      cells_node2 <- ref_metadata$cell_label[ref_metadata[,metadata_cluster_label] %in% node2_tip_nodes]
      ...
      subset_dataset <- ref_bpcells[c(cells_node1, cells_node2), matchup_marker_genes]
      ...
      subset_dataset <- subset_dataset %>% write_matrix_memory(compress=FALSE)
      #perform pca
    ...
    expect_equal(marker_ge_pca$x %>% nrow(), length(cells_node1) + length(cells_node2))
  })
      }
...
GetBPCellsSampleDataset <- function() {
  ...
  #remove genes with multiple ENS IDs that map to a gene (~40 ENS Ids removed)
...
AlignBPCellsObjs <- function(bpcells_obj1, bpcells_obj2) {
  keep_genes = intersect(rownames(bpcells_obj1), rownames(bpcells_obj2))
...
Classify <- function(bpcells_query, models, tree_struc) {
  ...
  # save to disk to make it quick
  query_cells <- bpcells_query %>% write_matrix_dir(tempfile("mat"), compress = F)
...
```
  ...
  X188_batch2   X368_batch2 X45.G2_batch1 

Code run during testing
```{r}
dense_ref <-LoadDenseReferenceAtlas()
ref_metadata <- dense_ref@meta.data
ref_metadata$cell_label <- ref_metadata$sample_name
WriteBPCellsReferenceAtlas("ref_atlas_bpcells")
ref_bpcells <- LoadBPCellsObj("ref_atlas_bpcells")
pseq_sample <- GetBPCellsSampleDataset()
pseq_sample <- AlignBPCellsObjs(pseq_sample, ref_bpcells)
ref_bpcells <- AlignBPCellsObjs(ref_bpcells, pseq_sample)
tree_broad <- CreateBroadTree(ref_metadata)
markers <- FindMarkerGenes(ref_bpcells, ref_metadata, tree_broad)
models <- GetModels(markers, ref_bpcells, ref_metadata, tree_broad)
pseq_metadata <- Classify(pseq_sample, models, tree_broad)
```

```{r}
#rstudioapi::documentSave()
#knitr::purl("bpcells_utils_mapping.Rmd")
```
